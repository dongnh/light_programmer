...
from .matter_lib import MatterClient
...
def generate_lighting_config(ip_address, port, output_filename, api_key=None):
    ...
    def is_light(d):
        hw = (d.get("hardware_type") or "").lower()
        return "light" in hw or "on_off" in d.get("capabilities", [])

    def is_sensor(d):
        hw = (d.get("hardware_type") or "").lower()
        return "sensor" in hw or "occupancy" in d.get("capabilities", [])

    light_nodes = [d for d in devices if is_light(d)]
    sensor_nodes = [d for d in devices if is_sensor(d)]
