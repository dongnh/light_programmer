...
- **Moonlight** — a level between 0 and 1 drives the bulb's warm night-light channel. Opt-in: only a deliberate sub-one setpoint triggers it, never the ramp up to daylight.
- **`unoccupied` fallback** — an away-state window and level instead of plain off (e.g. an evening glow, dark overnight).
- **Rain override** — weather scales an already-on light's brightness and colour while a rain sensor is active.
